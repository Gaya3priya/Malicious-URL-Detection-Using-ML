...
def having_ip_address(url):
    ip_address_pattern = patterns.ipv4_pattern + "|" + patterns.ipv6_pattern
...
def shortening_service(url):
    match = re.search(patterns.shortening_services, url)
    return -1 if match else 1
...
def https_token(url):
    match = re.search(patterns.http_https, url)
...
def main(url):
    ...
    status.append(favicon(url, soup, hostname) if soup else dns)
    status.append(https_token(url))
    status.append(request_url(url, soup, hostname) if soup else dns)
    status.append(url_of_anchor(url, soup, hostname) if soup else dns)
    status.append(links_in_tags(url, soup, hostname) if soup else dns)
    status.append(sfh(url, soup, hostname) if soup else dns)
    status.append(submitting_to_email(soup) if soup else dns)
    ...
    try:
        ...
        response = ""
    status.append(website_redirect(response))
    status.append(mouseOver(response))
    status.append(rightClick(response))
    status.append(i_frame(soup) if soup else dns)

    status.append(-1 if dns == -1 else age_of_domain(domain))

    status.append(dns)

    status.append(web_traffic(soup) if soup else dns)
